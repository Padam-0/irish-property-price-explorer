...
def retrieve_stats(df):
    ...
    for sale in range(count):
        ...
        if df.iloc[sale]['PSD'] == 'greater than or equal to 38 sq metres and less than 125 sq metres':
    ...
    compressed_hist_data = compress_list(hist_data, 2000)
...
def get_age_stats(engine, ref_id):
    ...
    with engine.connect() as con:
        df = pd.read_sql("SELECT * FROM prac_sexagemarriage WHERE uid = '" + ref_id + "' AND year=2016;", con)
...
def retrieve_cso(data_list, ref_id):
    ...
    with engine.connect() as con:
        oc = pd.read_sql("SELECT occupied FROM prac_housing WHERE uid = '" + ref_id + "' AND year=2016;", con)['occupied'][0]
        unoc = pd.read_sql("SELECT unoccupied FROM prac_housing WHERE uid = '" + ref_id + "' AND year=2016;", con)['unoccupied'][0]
...
########################################################################################
    ...
    with open('../prac/homepage/static/homepage/data/country_data.txt', 'w') as f:
...
########################################################################################
    ...
    with open('../prac/homepage/static/homepage/data/country_data_nobad.txt', 'w') as f:
        f.write(str(data_list))

    print("Done country nobad")
...
########################################################################################
    ...
    with open('../prac/homepage/static/homepage/data/leinster_data.txt', 'w') as f:
        f.write(str(data_list))
...
########################################################################################
    ...
    with open('../prac/homepage/static/homepage/data/leinster_data_nobad.txt', 'w') as f:
        f.write(str(data_list))

    print("Done leinster nobad")
########################################################################################
    region =  'munster'
    bad_data = 'true'
    ...
    with open('../prac/homepage/static/homepage/data/munster_data.txt', 'w') as f:
        f.write(str(data_list))
...
########################################################################################
    ...
    with open('../prac/homepage/static/homepage/data/munster_data_nobad.txt', 'w') as f:
        f.write(str(data_list))

    print("Done Munster nobad")
########################################################################################
    calcArea = 'county'
    county = 'dublin'
    ...
    with open('../prac/homepage/static/homepage/data/dublin_data.txt', 'w') as f:
        f.write(str(data_list))
...
########################################################################################
    ...
    data_list = retrieve_cso(data_list, 'C02')
    with open('../prac/homepage/static/homepage/data/dublin_data_nobad.txt', 'w') as f:
        f.write(str(data_list))
...
def refresh_cached_natave():
    ...
    with open('../prac/reporter/static/reporter/js/sales_natave.js', 'w') as f:
    ...
    with open('../prac/reporter/static/reporter/js/sales_natave_nbd.js', 'w') as f:
...
def get_na_data(df):
    ...
    na_sales = list(map(lambda x, y: [calendar.timegm(x.to_pydatetime().timetuple()) * 1000, round(float(y), 2)], nasdf.index.tolist(), nasdf.values))
    navdf = df.resample('M').count().dropna(axis=0, how='any')
    navdf = navdf[navdf['price'] != 0]
    na_volume = list(map(lambda x, y: [calendar.timegm(x.to_pydatetime().timetuple()) * 1000, round(float(y), 2)], navdf.index.tolist(), navdf.values))
...
def main():
    gdf = pd.read_csv('./good_data_ed.csv')
    ...
    gdf.drop_duplicates(subset=['sale_date', 'address', 'postcode', 'county', 'price', 'nfma',
       'vat_ex', 'DoP', 'PSD', 'region', 'latitude', 'longitude', 'ed', 'quality'])

    bdf = pd.read_csv('./bad_data.csv')

    #123642, 61783 are probably genuine outliers, add them back in for geobased analysis
